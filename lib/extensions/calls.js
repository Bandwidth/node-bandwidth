export default function callExtensions(Calls) {
	...
	Calls.transfer = {
		execute(id, transferTo, options = {}, cancelToken = null) {
			return this.update(
				id,
				{state: 'transferring', transferTo, ...options},
				cancelToken
			);
		}
	};
	...
	Calls.enableRecording = {
		execute(id, enable = true, cancelToken = null) {
			return this.update(id, {recordingEnabled: enable}, cancelToken);
		}
	};
}
