...
function Client(host, userId, apiToken, secret) {
  ...
  debug("Creating Catapult client");
}
...
Client.prototype.changeCall = function (callId, data, callback){
  this.post(this.concatUserUrl(callsPath) + "/" + callId, data, function(error, response){
    if(error){
      callback(error, null)
    } else {
...
Client.prototype.hangUp = function (callId, callback){
  this.changeCall(callId, {state: 'completed'}, callback)
}

Client.prototype.startRecording = function (callId, callback){
  this.changeCall(callId, {recordingEnabled: true}, callback)
}

Client.prototype.stopRecording = function (callId, callback){
  this.changeCall(callId, {recordingEnabled: false}, callback)
}

Client.prototype.getRecording = function(recordingId, callback){
  this.get(this.concatUserUrl(recordingsPath), recordingId, null, function(error, response){
    if(error){
      callback(error, null)
    } else {
      var recording = JSON.parse(response.data);
      callback(null,recording)
    }
  })
}

Client.prototype.getRecordings = function(callId, callback){
  var url = null

  if( !callback ){
    // get all recordings if callId wasn't specified
    callback = callId;
    callId   = null
    url = this.concatUserUrl(recordingsPath)
  } else {
    // get all recordings related with call
    callId = "" + callId + "/recordings"
    url = this.concatUserUrl(callsPath)
  }

  this.get(url, callId, null, function(error, response){
    if(error){
      callback(error, null)
    } else {
      var recording = JSON.parse(response.data);
...
Client.prototype.updatePhoneNumber = function(numberId, parameters, callback){
  this.post(this.concatUserUrl(phoneNumbersPath) + "/" + numberId, parameters, callback);
}
  ...
  var req = http.request(httpOptions, function(res){
    ...
    res.on('end', function(){
      ...
      if( [200, 201, 202].indexOf(response.statusCode) < 0 ) {
        ...
        callback({statusCode:response.statusCode, message:"There was a problem with your request. " + error.details, data: error.data})
      } else {
...
Client.prototype.post = function (path, data, callback)
{
  ...
  debug("data json " + content + " for path: " + path);
