...
var Message = function (client) {
	 ...
	 * @param  {Object} params Parameters for sending a new message, or an array of such parameters.
	 ...
	 * `none` `all` `error`
	 * @param  {Function} [callback] A callback for the new message object, if one message is sent;
	 * an array of objects, if multiple messages are set.
	 * @return {MessageResponse} A promise for the new message object, if one message is sent;
	 * an array of objects, if multiple messages are set.
	 */
	this.send = function (params, callback) {
		...
		.then(function (response) {
			var message;
			var location;
			if (response.statusCode === 201) { // Single message case
				message = params;
				location = response.headers.location;
				message.id = location.substring(location.lastIndexOf("/") + 1);
				return message;
			}
			else { //Multiple message case
				var failed = [];
				var successful = [];
				for (var k = 0; k < params.length; k++) {
					message = params[k];
					if (response.body[k].result === "accepted") {
						location = response.body[k].location ;
						message.id = location.substring(location.lastIndexOf("/") + 1);
						successful.push({
							result  : "accepted",
							message : message
						...
						message.error = response.body[k].error;
						failed.push({
							result  : "failed",
							message : message
						});
