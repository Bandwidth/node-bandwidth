...
var Call = function (client) {
	 ...
	 * @example 
	 * // This example creates a call and logs its status.
	 * client.Call.create({
	 *   from                 : process.env.myCatapultNumber,
	 *   to                   : process.env.myTargetNumber,
	 *   recordingEnabled     : true,
	 *   tag                  : "customer-service-call"
	 * })
	 * .then(function(response)){
	 *   return client.Call.get(response.id)
	 *   .then(function(response){
	 ...
	 * @example 
	 * // Place a call and check its status.
	 * client.Call.create({
	 *   from                 : process.env.myCatapultNumber,
	 *   to                   : process.env.myTargetNumber,
	 *   recordingEnabled     : true,
	 ...
	 * @example
	 * //Checks the state of the most recent call from a certain number.
	 * client.Call.list({
	 *   from      : process.env.myCatapultNumber,
	 *   size      : 1,
	 *   sortOrder : "desc"
 ...
 /**
	...
	* Many use cases are covered by other functions within Call.
	* @param {String} callId The ID of the call to update.
	* @param {Object} params The parameters to update.
	* @param {String} [params.state] One of 'rejected', 'active', 'completed', or
	* 'transferring'. Consider using Call.reject(), Call.transfer(), or
	* Call.hangup() to change the state of your call.
	* @param {Boolean} [params.recordingEnabled] Indicates if the call should
	* be recorded. Consider using Call.enableRecording() to enable recording.
	* @param {String} [params.transferTo] Number to transfer the call to; use
	* in conjunction with 'state : "transferring"'.
	* @param {String} [params.transferCallerId] The caller id that will be used
	* when the call is transferred. Allowed values are "private", any Bandwidth
	* number owned by the user, or, for incoming calls only, the incoming call
	* "from" number. Use in conjunction with 'state: "transferring"'.
	* @param {String} [params.whisperAudio] Audio to play to the caller that the
	* call will be transfered to. Use in conjunction with 'state: "transferring"'.
	* @param {String} [params.callbackUrl] Server URL to send call events to for
	* the transferred call. Use in conjunction with 'state: "transferring"'.
	* @returns {Promise} A promise for the operation that resolves to undefined.
	* @example 
	* // Enables recording on a call and transfers it.
	* client.Call.update("someCallId", {
	*   state            : "transferring",
	*   recordingEnabled : true,
	*   transferTo       : "+19195551212",
	*   transferCallerId : "private",
	*   callbackUrl      : "http://www.example.io/transferredCall"
	* });
	*/
	 ...
	 * @return {Promise} A promise for the operation, resolving to undefined.
	 * @example
	 * //Promise
...
// Private helper function for several audio-related functions.
	 ...
	 * @param {String} callId The ID of the call to enable recording on.
	 * @param {Function} [callback] The callback for the request.
	 * @returns {undefined}
	 * @example
	 * // Enable recording on a call.
	 * client.Call.enableRecording("someCallId");
	 */
...
// TODO: Add example.
	/**
	 * Complete the gather. This prevents additional 
	 * @param {String} callId The ID of the call
	 * @param {String} gatherId The ID of the gather
...
// TODO: Decide if audioApi ought to be private.
module.exports = Call;
