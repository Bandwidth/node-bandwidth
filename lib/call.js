...
var Call = function (client) {
	 ...
	 * @param {string} [params.state] The call state. Possible values: `rejected` to reject not answer, `active` to answer the call, `completed` to hangup the call, `transferring` to start and connect call to a new outbound call.
	 * @param {string} [params.recordingEnabled] Indicates if the call should be recorded. Values `true` or `false`. You can turn recording on/off and have multiple recordings on a single call.
	 * @param {string} [params.recordingFileFormat] The file format of the recorded call. Supported values are `wav` (default) and `mp3`.
	 * @param {string} [params.transferTo] Phone number or SIP address that the call is going to be transferred to.
	 * @param {string} [params.transferCallerId] This is the caller id that will be used when the call is transferred. This parameter is only considered in `transfer` state. <br> - transferring an incoming call: allowed values are 1) "private" 2) the incoming call "from" number or 3) any Bandwidth number owned by user. <br> - transferring an outgoing call call: allowed values are 1) "private" or 2) any Bandwidth phone number owned by user.
	 * @param {string} [params.whisperAudio] Audio to be played to the caller that the call will be transferred to.
	 * @param {string} [params.callbackUrl] The server URL where the call events for the new call will be sent upon transferring.
